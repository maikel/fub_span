...
#include "fub/span/error_policy/throw_exception.hpp"
...
{
	REQUIRE(( !ranges::DefaultConstructible<span<int, 1>>() ));
}
...
using span_ = fub::span<T, Dim, error_policy::throw_exception<>>;
...
TEST_CASE("subspan")
{
	...
	SECTION("static length")
	{
		...
		REQUIRE(( ranges::Same<decltype(sub), span<int, 4>>() ));
	...
	SECTION("dynamic length")
	{
		...
		REQUIRE(( ranges::Same<decltype(sub), span<int>>() ));
		REQUIRE( sub.data() == vec.data() + 1 );
		REQUIRE( sub.size() == 4 );
		REQUIRE( sub[0] == 1 );
	}
	...
	{
		auto sub = take<10>(s);
		REQUIRE(( ranges::Same<decltype(sub), span<int, 10>>() ));
		REQUIRE(s == sub);

		auto d1 = drop<3>(s);
		auto d2 = drop<3>(sub);
		REQUIRE(( ranges::Same<decltype(d1), span<int>>() ));
		REQUIRE(( ranges::Same<decltype(d2), span<int, 7>>() ));
		REQUIRE(d1 == d2);
	}
}
